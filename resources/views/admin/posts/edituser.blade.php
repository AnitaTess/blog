...
<form method="POST" action="/admin/users/{{ $user->id }}" class="bg-white p-6 rounded-xl border border-pink-200 mb-8 space-x-4">
@csrf
@method ('PATCH')
    <h2 class="ml-4 text-xl font-bold">Edit this user's admin status: {{ $user->username }}</h2>
  <div class="mt-6">
  <label for="title">Edit Admin Status:</label>
<input class="border border-gray-400 p-2 w-full" type="number" min="0" max="1" name="is_admin" id="is_admin" value="{{ $user->is_admin }}" required>
</div>
<div class="mt-6">
