...
<form method="POST" action="/admin/posts/{{ $post->id }}" enctype="multipart/form-data" class="bg-white p-6 rounded-xl border border-pink-200 mb-8 space-x-4">
@csrf
@method ('PATCH')
    <h2 class="ml-4 text-xl font-bold">Edit the topic: {{ $post->title }}</h2>
  <div class="mt-6">
...
<div class="mt-6">
  <label for="body">Edit what is this topic about:</label>
<input class="border border-gray-400 p-2 w-full" type="text" name="body" id="body" value="{{ $post->body }}" required>
</div>
<div class="mt-6">
  <label for="body">Edit a thumbnail:</label>
<input class="border border-gray-400 p-2 w-full" type="file" name="thumbnail" id="thumbnail">
<img src="{{ asset('storage/' . $post->thumbnail) }}" alt="thumbnail" width="200" height="200" class="mt-2">
</div>
  <div class="justify-end flex mt-2">
  <button class="bg-pink-400 text-white rounded py-2 px-4 hover:bg-pink-600" type="submit">Update</button>
</div>
</form>
</div>
</section>
