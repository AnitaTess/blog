...
<a href="/admin/posts/allposts" class="text-xl font-bold hover:text-pink-800">Back to Post Management</a>
</div>
...
<div class="text-sm font-medium text-gray-900">
  <a href="/users/{{ $user->id }}">
   Admin rank: {{ $user->is_admin }}
</a>
</div>
</td>
@can('superadmin')
<td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
  <a href="/admin/users/{{ $user->id }}/useredit" class="text-pink-600 hover:text-pink-800">Edit</a>
</td>
<td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
<form method="POST" action="/admin/users/{{ $user->id }}">
@csrf
@method ('DELETE')
<button onclick="return confirm('Are you sure you want to delete {{ $user->username }} ?')" class="text-xs text-red-600 border border-red-600 rounded p-2">
Delete</button>
</form>
</td>
@endcan
